...
use Chatbox\WebhookProxy\Http\Actions\WebhookDeleteAction;
...
use Chatbox\WebhookProxy\Http\Actions\WebhookUpdateAction;
...
class WebhookProxyServiceProvider extends ServiceProvider {
	...
	public function boot(  ) {
		...
		app()->router->group(["prefix"=>"/api/"],function ($route) {
			...
			$route->delete('webhook/destroy/{name}', WebhookDeleteAction::class.'@handle');
			$route->post('webhook/update', WebhookUpdateAction::class.'@handle');
		});
